...
Return ONLY valid JSON with this exact structure:
{
  ...
  "overallCohesion": ${isSingleItem ? 'Score overall design quality (0-100)' : '82'}${
    userProfile
      ? `,
  "profileSpecificFeedback": "DETAILED paragraph (5-8 sentences) explaining HOW WELL this ${isSingleItem ? 'item' : 'outfit'} works specifically for ${userProfile.name}. Be inclusive and accepting of personal style choices while being honest about practical fit considerations. IMPORTANT FLAGS TO CHECK: (1) **Gender/Context Note**: If the ${isSingleItem ? 'item' : 'outfit'} is traditionally gendered (e.g., women's bralette for a male profile), acknowledge this neutrally: \"This piece is traditionally designed for [gender], which means [specific fit/function considerations for their body]. Fashion is personal, so if you love it, here's how to wear it confidently: [tailored suggestions].\" (2) **Skin Tone & Color**: Does the color work with their ${userProfile.skinAnalysis?.skinTone} skin tone and ${userProfile.skinAnalysis?.undertone} undertone? Be explicit about flattery. (3) **Body Shape Fit**: Given their ${userProfile.buildType} body shape, how does the ${isSingleItem ? 'fit and silhouette' : 'fit and silhouettes'} work? Acknowledge if it's designed differently but explain how to style it to suit them. (4) **Aesthetic Alignment**: Does it align with their ${userProfile.aesthetics?.join(', ')} aesthetic? Flag if it conflicts. (5) **Lifestyle Formality**: Does the formality level work for their ${userProfile.formality} lifestyle? (6) **Warm Closure**: End with honest, encouraging recommendations on how to make it work for THEIR body and style. Respect personal choice while being practical about fit and function."`
      : ''
  }
}
