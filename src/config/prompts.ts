...
Return ONLY valid JSON with this exact structure:
{
  ...
  "overallCohesion": ${isSingleItem ? 'Score overall design quality (0-100)' : '82'}${
    userProfile
      ? `,
  "profileSpecificFeedback": "Answer in this exact format: '[Yes/No] — [one sentence]'. You must choose Yes or No — no middle ground. Choose Yes if the dominant effect is positive for ${userProfile.name}, No if it isn't. The single sentence must name one specific reason tied to their ${userProfile.skinAnalysis?.skinTone} skin tone, ${userProfile.buildType} build, or ${userProfile.aesthetics?.join('/')} aesthetic. No more than 25 words after the dash."`
      : ''
  }
}
