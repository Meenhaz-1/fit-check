...
### 4. Start (Database Initializes Automatically)
The database is initialized automatically when the app starts. No manual setup needed.
...
**Phase 1a uses a simple file-based system:**

### Sample Wardrobe (CSV)
- **File:** `data/sample-wardrobe.csv`
- **Content:** 10 pre-populated wardrobe items
- **Used by:** All testers see the same wardrobe
- **Editable:** You can modify the CSV to add/change items

### Evaluations (JSON)
- **File:** `data/evaluations.json`
- **Content:** All user evaluations and verdicts
- **Persists:** Data survives server restarts
- **Inspectable:** Easy to view/debug raw data

**Why CSV + JSON for Phase 1a?**
- ✅ Zero dependencies (native Node.js file APIs)
- ✅ Data persists between restarts (great for testing)
- ✅ Easy to inspect/edit raw data
- ✅ Can pre-populate sample data
- ✅ No compilation or complex setup
- 📝 Phase 2 will switch to SQLite when we need scalability
...
- Database resets on server restart (OK for Phase 1a MVP testing)
